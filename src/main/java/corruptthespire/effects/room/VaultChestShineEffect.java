...
public class VaultChestShineEffect extends AbstractGameEffect {
    ...
    public VaultChestShineEffect(float x, float y, boolean corrupted) {
        ...
        this.x = x + MathUtils.random(-200.0F, 170.0F) * Settings.scale - (float)this.img.packedWidth / 2.0F;
        this.y = y + MathUtils.random(-250.0F, 50.0F) * Settings.scale - (float)this.img.packedHeight / 2.0F;
        this.vY = MathUtils.random(10.0F, 50.0F) * Settings.scale;
        this.alpha = MathUtils.random(0.7F, 1.0F);
        this.color = corrupted ? new Color(0.7F, 0.13F, MathUtils.random(0.1F, 0.4F), this.alpha) : new Color(1.0F, 1.0F, MathUtils.random(0.6F, 0.9F), this.alpha);
        this.scale = 0.01F;
