        ...
        for (AbstractMonster m : AbstractDungeon.getCurrRoom().monsters.monsters) {
            if (m != this.owner && !m.isDying && !m.halfDead && !m.isDeadOrEscaped()) {
                if (AbstractDungeon.actionManager.turnHasEnded) {
                    ...
                    this.addToBot(new ApplyPowerAction(m, this.owner, new StrengthPower(m, this.amount), this.amount));
                }
                this.addToBot(new ApplyPowerAction(m, this.owner, new MetallicizePower(m, this.amount), this.amount));
            }
        }
    }
}
